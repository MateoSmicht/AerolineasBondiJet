...
public class Publico extends Vuelo {
    ...
    public boolean elVueloEsNacional(Vuelo v) {
		...
		if (vuelo.aeropuertoDestino.getPais().equals("Argentina") && vuelo.aeropuertoSalida.getPais().equals("Argentina")) {
    ...
    public Vuelo vueloAlternativoPublico(Vuelo vueloAcancelar, Map<String, Vuelo> publicos) {
        ...
        LocalDate fechaVueloACancelar = LocalDate.parse(vueloComp.getFecha(), DateTimeFormatter.ofPattern("dd/MM/yyyy"));

        for (Vuelo vuelo : publicos.values()) {
            if (!vuelo.getIdentificacion().equals(vueloComp.identificacion) &&
